    ...
    <el-form label-width="100px" ref="dataForm" :model="dataForm" :rules="dataRule">
      ...
      <el-card class="box-card" shadow="never">
        <div slot="header" class="clearfix">
          <span>{{ $t('inventory.outbound.agent_info') }}</span>
        </div>
        <div class="text item">
      ...
      <el-card class="box-card mt10" shadow="never">
        <div slot="header" class="clearfix">
          <span>{{ $t('inventory.outbound.equipment_info') }}</span>
        </div>
        <div class="text item">
          ...
          <el-form-item :label="$t('inventory.outbound.total')" prop="total">
            <el-input-number :placeholder="$t('common.please_input')+$t('inventory.outbound.total')" v-model="dataForm.total"></el-input-number>
          </el-form-item>
  ...
  export default {
    ...
    data() {
      return {
        ...
        {
          member_id: [
            { required: true, message: this.$t('inventory.outbound.rules.member_id.require'), trigger: 'blur' },
          ],
          type: [
    ...
    methods: {
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/outbound/first_step`),
              method: 'post',
